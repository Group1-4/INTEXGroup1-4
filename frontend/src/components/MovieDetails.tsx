...
const MovieDetails: React.FC<Props> = ({ movieId: initialMovieId }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        const { movie: movieData, userRating: rating } = await fetchUserMovieDetails(currentMovieId);
        setMovie(movieData);
  ...
  const handleRatingChange = async (newRating: number | null) => {
    ...
    if (newRating !== null) {
      try {
        const response = await rateMovie(currentMovieId, newRating);
  ...
  if (loading) {
    return <div className="min-h-screen d-flex align-items-center justify-content-center bg-image text-white page-container">Loading movie details...</div>;
  }
  if (error) {
    return <div className="min-h-screen d-flex align-items-center justify-content-center bg-image text-danger page-container">Error: {error}</div>;
  }
  ...
  const yearDisplay = movie.releaseYear ?? "Unknown";
  const ratingDisplay = movie.rating ?? "Unknown";
  const durationDisplay = movie.duration ?? "Unknown";
  const directorDisplay = movie.director ?? "Unknown";
  const castDisplay = movie.cast ?? "Unknown";
  const descriptionDisplay = movie.description ?? "Unknown";
      ...
      <div className="container">
        <div className="row mb-4 rounded p-3" style={{ backgroundColor: creamOpaque }}>
          <div className="col-md-4">
            <div className="rounded shadow" style={{ backgroundColor: creamOpaque }}>
              <img
                ...
                onError={(e) => { (e.currentTarget as HTMLImageElement).src = "./fallback-poster.png"; }}
          ...
          <div className="col-md-8 position-relative" style={{ backgroundColor: creamOpaque, padding: "1.5rem" }}>
            ...
            <button className="btn" style={{ backgroundColor: turquoise, color: "white", marginBottom: "10px" }} onClick={handleWatchNowClick}>Watch Now</button>
